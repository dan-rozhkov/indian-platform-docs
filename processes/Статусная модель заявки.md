...
| 3 | Пресейл / КП | `kp_calculated`, `kp_uploaded`, `kp_approved?`, `kp_returned?` | Система, Партнёр Индии, Админ |
...
> [!warning] Блок КП остаётся частично спорным
> В ФТ от 6 апреля появились новые пункты по КП и предварительному расчёту, но обязательность полного цикла согласования КП всё ещё помечена как неуточнённая. Поэтому события `kp_approved` и `kp_returned` не следует трактовать как универсальные для всех заявок.
